...
func exportToTomlFile() {
	...
	builder.AddEntryPoint("web", ":8000")
	builder.AddEntryPoint("websecure", ":8443")
	cmd.ExportToml(builder.GetConfiguration(), f)
	if err != nil {
		fmt.Printf("Failed to export Traefik configuration in %s: %q\n", f.Name(), err.Error())
		return
	}
...
func exportToYamlFile() {
	...
	builder.AddEntryPoint("web", ":8000")
	builder.AddEntryPoint("websecure", ":8443")
	cmd.ExportYaml(builder.GetConfiguration(), f)
	if err != nil {
		fmt.Printf("Failed to export Traefik configuration in %s: %q\n", f.Name(), err.Error())
		return
	}
...
func exportToCLI() {
	...
	builder.AddEntryPoint("web", ":8000")
	builder.AddEntryPoint("websecure", ":8443")
	cmd.ExportCLI(builder.GetConfiguration(), os.Stdout)
	if err != nil {
		fmt.Printf("Failed to export Traefik configuration: %q\n", err.Error())
		return
	}

	fmt.Print("Successfully exported Traefik configuration\n")
}
